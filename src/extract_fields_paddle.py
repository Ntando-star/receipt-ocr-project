...
NOISE_KEYWORDS = [  
    'ITEM', 'TOTAL', 'CASHIER', 'WWW', 'PAY', 'INVOICE', 'DATE', 'TIME',   
    'DOCUMENT', 'MEMBER', 'AMOUNT', 'DISC', 'PRICE', 'QTY', 'CODE/DESC'  
]  
...
def extract_fields(text):  
    ...
    Optimized for PaddleOCR outputs.  
    """  
    # --- Preprocess OCR text ---  
    ...
    if not address_lines:  
        for line in lines:  
            if re.search(r'\d', line) and re.search(r'[A-Za-z]', line):  
                address_lines.append(line.strip())  
                if len(address_lines) >= 3:  # limit fallback to 3 lines for address  
                    break  
